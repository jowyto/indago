 ...
 *
 * @method DatasetQuery leftJoinDatasetValoracion($relationAlias = null) Adds a LEFT JOIN clause to the query using the DatasetValoracion relation
 * @method DatasetQuery rightJoinDatasetValoracion($relationAlias = null) Adds a RIGHT JOIN clause to the query using the DatasetValoracion relation
 * @method DatasetQuery innerJoinDatasetValoracion($relationAlias = null) Adds a INNER JOIN clause to the query using the DatasetValoracion relation
 *
...
abstract class BaseDatasetQuery extends ModelCriteria
{
    ...
    /**
     * Filter the query by a related DatasetValoracion object
     *
     * @param   DatasetValoracion|PropelObjectCollection $datasetValoracion  the related object to use as filter
     * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
     *
     * @return   DatasetQuery The current query, for fluid interface
     * @throws   PropelException - if the provided filter is invalid.
     */
    public function filterByDatasetValoracion($datasetValoracion, $comparison = null)
    {
        if ($datasetValoracion instanceof DatasetValoracion) {
            return $this
                ->addUsingAlias(DatasetPeer::ID, $datasetValoracion->getDatasetId(), $comparison);
        } elseif ($datasetValoracion instanceof PropelObjectCollection) {
            return $this
                ->useDatasetValoracionQuery()
                ->filterByPrimaryKeys($datasetValoracion->getPrimaryKeys())
                ->endUse();
        } else {
            throw new PropelException('filterByDatasetValoracion() only accepts arguments of type DatasetValoracion or PropelCollection');
        }
    }

    /**
     * Adds a JOIN clause to the query using the DatasetValoracion relation
     *
     * @param     string $relationAlias optional alias for the relation
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return DatasetQuery The current query, for fluid interface
     */
    public function joinDatasetValoracion($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        $tableMap = $this->getTableMap();
        $relationMap = $tableMap->getRelation('DatasetValoracion');
        ...
        if ($relationAlias) {
            ...
            $this->addJoinObject($join, 'DatasetValoracion');
        }
    ...
    /**
     * Use the DatasetValoracion relation DatasetValoracion object
     *
     * @see       useQuery()
     *
     * @param     string $relationAlias optional alias for the relation,
     ...
     * @param     string $joinType Accepted values are null, 'left join', 'right join', 'inner join'
     *
     * @return   DatasetValoracionQuery A secondary query class using the current class as primary query
     */
    public function useDatasetValoracionQuery($relationAlias = null, $joinType = Criteria::LEFT_JOIN)
    {
        return $this
            ->joinDatasetValoracion($relationAlias, $joinType)
            ->useQuery($relationAlias ? $relationAlias : 'DatasetValoracion', 'DatasetValoracionQuery');
    }
